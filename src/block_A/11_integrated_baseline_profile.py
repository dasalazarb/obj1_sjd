...
def setup_logging() -> logging.Logger:
    log_dir = common.OUTPUTS_DIR / "logs"; log_dir.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    logger=setup_logging(); common.ensure_output_dirs(); tables=common.BLOCKA_TABLES_DIR; qc_dir=common.BLOCKA_QC_DIR; fig_dir=common.OUTPUTS_DIR/"figures"/"blockA"; [p.mkdir(parents=True,exist_ok=True) for p in (tables,qc_dir,fig_dir,common.INTERMEDIATE_DATA_DIR)]
    ...
    baseline_out.to_parquet(common.INTERMEDIATE_DATA_DIR/"11_integrated_baseline_patient_level.parquet",index=False); baseline_out.to_csv(common.INTERMEDIATE_DATA_DIR/"11_integrated_baseline_patient_level.csv",index=False)
    ...
    generated=sorted([p.relative_to(PROJECT_ROOT).as_posix() for p in [*tables.glob("11_*"),*qc_dir.glob("11_*"),*fig_dir.glob("11_*"),common.INTERMEDIATE_DATA_DIR/"11_integrated_baseline_patient_level.parquet",common.INTERMEDIATE_DATA_DIR/"11_integrated_baseline_patient_level.csv",common.OUTPUTS_DIR/"logs"/"11_integrated_baseline_profile.log"]])
