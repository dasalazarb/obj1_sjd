...
CONDITIONS = [
    ConditionSpec("fibromyalgia", "Fibromyalgia", ("rheumatological_comorbidities__fibromyalgia1", "rheumatological_comorbidities__fibromyalgia1_hx", "rheumatological_comorbidities__fibromyalgia1_confirm"), notes="Primary definition uses any diagnosis/history/confirmation source."),
    ConditionSpec("osteoporosis", "Osteoporosis", ("rheumatological_comorbidities__osteoporosis1", "rheumatological_comorbidities__osteoporosis1_hx", "rheumatological_comorbidities__osteoporosis1_confirm")),
    ConditionSpec("ild", "Interstitial lung disease", ("past_medical_history__respiratory_hx_lung", "past_medical_history__resp_hx_lung_fibro"), ("sjogren's_syndrome_disease_damage_index__fibrosis_interstitial",)),
    ConditionSpec("thyroid_disease", "Thyroid disease", ("past_medical_history__thyroid_disease",), ("sjogren's_syndrome_history__thyroiditis",), notes="Patient history only; family history intentionally excluded."),
    ConditionSpec("depression", "Depression", ("past_medical_history__neuro_hx_depression",), ("systems_review_for_physician__psych_depr",)),
    ConditionSpec("anxiety", "Physician-recorded anxiety symptoms", ("systems_review_for_physician__adr_q22_anxiety",), ("ans__anxiety", "ans__anxiety_severity", "autonomic_nervous_system_questionnaire__anxiety", "autonomic_nervous_system_questionnaire__anxiety_severity"), notes="Symptom indicator, not a confirmed diagnosis."),
    ConditionSpec("raynaud", "Raynaud's phenomenon", ("rheumatological_comorbidities__integ_raynds", "rheumatological_comorbidities__integ_raynds_hx", "rheumatological_comorbidities__integ_raynds_confirm"), ("sjogren's_syndrome_history__raynaud_phenom",)),
    ConditionSpec("peripheral_neuropathy", "Peripheral neuropathy", ("past_medical_history__neuro_hx_neuropathy",), ("sjogren's_syndrome_disease_damage_index__neuro_cran_periph",)),
    ConditionSpec("renal_tubular_acidosis", "Renal tubular acidosis", ("past_medical_history__renal_hx_tubular_acid",)),
    ConditionSpec("myositis", "Myositis", ("rheumatological_comorbidities__polymyositis", "rheumatological_comorbidities__polymyositis_hx", "rheumatological_comorbidities__polymyositis_confirm", "rheumatological_comorbidities__dermatomyositis", "rheumatological_comorbidities__dermatomyositis_hx", "rheumatological_comorbidities__dermatomyositis_confirm"), ("sjogren's_syndrome_history__myositis_myalgia",)),
    ConditionSpec("cryoglobulinemia", "Cryoglobulinemia", ("rheumatological_comorbidities__cryoglobulinemia", "rheumatological_comorbidities__cryoglobulinemia_hx", "rheumatological_comorbidities__cryoglobulinemia_confirm"), definition_type="other clinically relevant"),
    ConditionSpec("chronic_bronchitis", "Chronic bronchitis", ("past_medical_history__respiratory_hx_bronchitis",)),
]
CONDITION_NAMES = [c.name for c in CONDITIONS]
UNAVAILABLE_CONDITIONS = {
    "bronchiectasis": "No exact physician-documented bronchiectasis diagnosis variable; chronic bronchitis was not substituted.",
    "psoriatic arthritis": "No exact psoriatic arthritis variable identified; psoriasis/family history were not substituted.",
    "hypokalemia": "No physician-documented baseline hypokalemia diagnosis variable; single potassium results were not substituted.",
    "gastritis": "No exact gastritis diagnosis variable; nonspecific GI other fields were not substituted.",
    "neutropenia": "No baseline neutropenia diagnosis variable; hematologic ESSDAI activity was not substituted.",
}
...
def load_selected_raw_columns(path: Path) -> pd.DataFrame:
    ...
    for c in CONDITIONS: needed.update(c.primary); needed.update(c.sensitivity)
...
def derive_comorbidity_indicators(raw: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    out = raw[["patient_id", "visit_date"]].copy(); bool_cols=[]
    source_rows=[]
    for spec in CONDITIONS:
        norm_cols=[]
        for col in spec.primary + spec.sensitivity:
            if col in raw:
                ncol = f"src__{col}"; out[ncol] = normalize_binary_flag(raw[col]); norm_cols.append(ncol); bool_cols.append(ncol)
        primary_cols = [f"src__{c}" for c in spec.primary if f"src__{c}" in out]
        out[spec.name] = nullable_or(out[primary_cols]) if primary_cols else pd.Series(pd.NA, index=out.index, dtype="boolean")
        bool_cols.append(spec.name)
        sens_cols = [f"src__{c}" for c in spec.sensitivity if f"src__{c}" in out]
        if sens_cols:
            out[f"{spec.name}_sensitivity"] = nullable_or(out[primary_cols + sens_cols]); bool_cols.append(f"{spec.name}_sensitivity")
        source_rows.append({"condition": spec.name, "primary_source_columns": ";".join(spec.primary), "sensitivity_source_columns": ";".join(spec.sensitivity), "derivation_rule": "nullable OR across valid baseline sources", "definition_type": spec.definition_type, "availability": "available" if primary_cols else "unavailable", "n_unrecognized_values": sum(1 for r in UNRECOGNIZED if r["column"] in spec.primary + spec.sensitivity)})
    # extra QC subtypes/sources
    for name, col in {"fibromyalgia_history":"rheumatological_comorbidities__fibromyalgia1_hx", "fibromyalgia_confirmed":"rheumatological_comorbidities__fibromyalgia1_confirm"}.items():
        if col in raw: out[name]=normalize_binary_flag(raw[col]); bool_cols.append(name)
    if "past_medical_history__neuro_hx_neuropathy_spfy" in raw:
        sub = raw["past_medical_history__neuro_hx_neuropathy_spfy"].fillna("").astype(str).str.lower()
        out["sensory_neuropathy"] = sub.str.contains("sensory").astype("boolean")
        out["motor_neuropathy"] = sub.str.contains("motor").astype("boolean")
        out["cranial_nerve_involvement"] = sub.str.contains("cranial").astype("boolean")
        out["sensory_motor_neuropathy"] = (out["sensory_neuropathy"] & out["motor_neuropathy"]).astype("boolean")
        bool_cols += ["sensory_neuropathy","motor_neuropathy","cranial_nerve_involvement","sensory_motor_neuropathy"]
    collapsed, dup = collapse_same_patient_date(out, list(dict.fromkeys(bool_cols)), [])
    return collapsed, pd.DataFrame(source_rows)
...
def main() -> None:
    ...
    logging.info("[2/8] Building baseline comorbidity indicators"); ind,dup=derive_comorbidity_indicators(raw)
    ...
    create_dotplot(overall,FIGURES_DIR/"07_comorbidities_dotplot.pdf"); create_grouped_barplot(bypop,FIGURES_DIR/"07_comorbidities_grouped_bar.pdf"); create_progression_forestplot(prog, overall.condition.tolist(), FIGURES_DIR/"07_comorbidities_progression_forestplot.pdf")
    ...
    source=summarize_source_mapping=pd.DataFrame([{"condition":c.name,"primary_source_columns":";".join(c.primary),"sensitivity_source_columns":";".join(c.sensitivity),"derivation_rule":"nullable OR across valid baseline sources; baseline visit only","definition_type":c.definition_type,"availability":"available","n_unrecognized_values":0} for c in CONDITIONS]); source.to_csv(QC_DIR/"07_comorbidities_source_mapping.csv",index=False)
    ...
    top=overall.head(3); ild=overall[overall.condition.eq('ild')].iloc[0]
    claim=f"{top.iloc[0].display_label} was the most prevalent documented baseline comorbidity ({top.iloc[0].pct_total_cohort:.1f}%), followed by {top.iloc[1].display_label} ({top.iloc[1].pct_total_cohort:.1f}%) and {top.iloc[2].display_label} ({top.iloc[2].pct_total_cohort:.1f}%). Interstitial lung disease was documented in {ild.pct_total_cohort:.1f}% of patients. Depression and anxiety are independent indicators and patients can appear in both."
    logging.info(claim)
