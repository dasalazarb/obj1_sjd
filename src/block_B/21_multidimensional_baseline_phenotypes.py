...
def main(argv=None):
    ...
    outputs=[common.INTERMEDIATE_DATA_DIR/"21_multidimensional_baseline_phenotypes"/"21_multidimensional_baseline_patient.parquet",common.INTERMEDIATE_DATA_DIR/"21_multidimensional_baseline_phenotypes"/"21_multidimensional_baseline_patient.csv",common.BLOCKB_TABLES_DIR/"21_multidimensional_baseline_phenotypes"/"21_multidimensional_profile_by_pop.csv",common.BLOCKB_TABLES_DIR/"21_multidimensional_baseline_phenotypes"/"21_variable_denominators.csv",common.BLOCKB_TABLES_DIR/"21_multidimensional_baseline_phenotypes"/"21_adjusted_pop_comparisons.csv",common.BLOCKB_FIGURES_DIR/"21_multidimensional_baseline_phenotypes"/"21_multidimensional_baseline_heatmap.pdf",common.BLOCKB_QC_DIR/"21_multidimensional_baseline_phenotypes"/"21_multidimensional_baseline_qc.json",common.BLOCKB_QC_DIR/"21_multidimensional_baseline_phenotypes"/"21_merge_summary.csv"]
    if not args.overwrite and any(p.exists() for p in outputs): raise FileExistsError("Output exists; use --overwrite")
    common.ensure_output_dirs(); [p.parent.mkdir(parents=True, exist_ok=True) for p in outputs]; log_dir=common.BLOCKB_LOGS_DIR/"21_multidimensional_baseline_phenotypes"; log_dir.mkdir(parents=True, exist_ok=True); logging.basicConfig(level=logging.INFO,format="%(asctime)s %(levelname)s %(message)s",handlers=[logging.FileHandler(log_dir/"21_multidimensional_baseline_phenotypes.log",mode="w"),logging.StreamHandler()])
